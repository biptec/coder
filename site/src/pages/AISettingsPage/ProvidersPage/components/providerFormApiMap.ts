...
const buildBedrockSettings = (
	...
	// Mantle is a passthrough transport: the client sends the model, so the
	// provider omits the model fields. The endpoint discriminator is only
	// emitted for mantle to keep InvokeModel blobs minimal (an absent
	// endpoint resolves to InvokeModel server-side).
...
export const aiProviderToFormValues = (
	...
	if (isBedrockProvider(provider)) {
		const s = (provider.settings as SettingsWire | null) ?? {};
		// A missing endpoint resolves to InvokeModel (legacy rows). Mantle
		// providers store no model fields, so leave them blank.
		const endpoint: AIProviderBedrockEndpoint =
			s.endpoint === "mantle" ? "mantle" : "invoke-model";
		const isMantle = endpoint === "mantle";
		return {
			type: "bedrock",
			name: provider.name,
