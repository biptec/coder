...
// WorkspaceProcessStartResult preserves the legacy/full-catalog start response.
// The assistant-facing start_process uses WorkspaceProcessResult instead.
...
One MCP call is bounded by the deployment-wide MCP tool timeout across workspace readiness and process-start acknowledgement; this request ceiling never controls the durable process lifetime. If this call ends with a timeout, 502, disconnect, or any other uncertain result after process submission, DO NOT start the command again. First call coder_workspace_process_list for the same workspace and recover the existing process by matching its command, workdir, and start time.
