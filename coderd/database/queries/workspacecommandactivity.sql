...
WHERE workspace_id = sqlc.arg(workspace_id)
  AND (
    sqlc.arg(id_search)::text = ''
    OR strpos(lower(id::text), lower(sqlc.arg(id_search)::text)) > 0
  )
  AND (
  ...
  AND (
    sqlc.narg(exit_code)::integer IS NULL
...
ORDER BY
  ...
  CASE WHEN sqlc.arg(sort_by)::text = 'source' AND sqlc.arg(sort_direction)::text = 'asc' THEN
    CASE
    ...
    END
  END ASC,
  CASE WHEN sqlc.arg(sort_by)::text = 'source' AND sqlc.arg(sort_direction)::text = 'desc' THEN
    CASE
      WHEN source = 'agentproc' AND tool IN (
        'exec', 'bash', 'process_start',
        'coder_workspace_exec', 'coder_workspace_bash',
